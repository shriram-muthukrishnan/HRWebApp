...
2. Load all tasks from ${modernization-work-folder}/tasks.json and execute them one by one in the order they appear in the `tasks` array in tasks.json (do not reorder tasks):
    ...
    - Choose the right custom agent to execute the task based on the `type` field of the task in tasks.json, and call the custom agent with the prompt according to the task type and information in tasks.json. The custom agent will return the execution result including whether the task is successful, skipped or failed, and a summary of the execution.
        ...
        For tasks with `"type": "integrationTest"` in tasks.json, call custom agent `modernize-azure-devops-engineer` with prompt:
                ```md
...
8. **MANDATORY: Playbook Evidence Validation** (only when playbook attachments are present):
   **You MUST complete this step if playbook attachments were provided.** Do NOT skip it.
   After final verification, handle playbook compliance evidence as a best-effort step that must not fail the overall modernization plan execution:
   - If `baseline-commit-sha` is `unknown`, do not call skill `validate-playbook-evidence`. Instead, write `${modernization-work-folder}/playbook-evidence.md` with a short report explaining that diff-based playbook evidence could not be generated because the baseline commit SHA was unavailable.
   - If playbook attachments are present and `baseline-commit-sha` is not `unknown`, call skill `validate-playbook-evidence` to analyze code changes and produce playbook compliance evidence:
     - baseline-commit-sha: the baseline commit SHA captured before task execution
     - playbook-file-list: comma-separated list of playbook file names from the attachments
     - evidence-output-path: `${modernization-work-folder}/playbook-evidence.md`
   - If the evidence validation skill fails or cannot produce evidence, write a short failure report to `${modernization-work-folder}/playbook-evidence.md` and continue completing the modernization plan without failing the run.
